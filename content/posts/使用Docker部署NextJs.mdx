...
你可以查看[官方文档](https://nextjs.org/docs/app/getting-started/deploying)了解更多的信息。

本文我将重点介绍如何使用 Docker 进行容器化部署，具体内容我会分步骤详细介绍，包括：
...
国内推荐使用 [腾讯云](https://cloud.tencent.com/product/cvm) 或者 [阿里云](https://cn.aliyun.com/product/ecs?from_alibabacloud=) 的云服务器，操作系统建议使用主流的 Linux 发行版，比如 Ubuntu 或 CentOS，配置方面建议最少使用 2 核 2G 以上的配置。

本网站目前使用的是阿里云的 ESC 云服务器，操作系统为 Ubuntu 22.04 LTS，配置为 2 核 2G。

##### 一个域名

国内推荐使用 [腾讯云](https://cloud.tencent.com/product/cvm) 或者 [阿里云](https://cn.aliyun.com/product/ecs?from_alibabacloud=) 的域名服务。
...
- 设置 **headers**，如果你使用了[流式响应](https://nextjs.org/docs/app/api-reference/file-conventions/loading)，那么需要设置 `X-Accel-Buffering` 为 **no**，这样可以避免 Nginx 的响应缓存。
...
如果你还不熟悉 Docker，可以点击[这里](https://jeewong.com/posts/docker-getting-started)。
...
先，你需要确保本地已经启动了 Docker Desktop 或者 OrbStack。
...
如果你使用的是阿里云，可以查看[阿里云 SSL 证书服务](https://cn.aliyun.com/product/cas?from_alibabacloud=)

如果你使用的是腾讯云，可以查看[腾讯云 SSL 证书服务](https://cloud.tencent.com/product/ssl)。

这里，我们以免费证书为例，进入到证书管理页面后，完成免费证书的申请。申请完成后，我们需要将将证书下载到本地。通常，在证书列表的操作一栏能找到下载按钮，我们选择下载类型为 Nginx。
...
> 在构建 Docker 镜像时，默认构建的是当前平台的镜像。如果需要构建特定平台的镜像，可以通过以下命令指定目标平台：
>
> ```bash
...
如果看到 **nextjs-app** 容器，那么说明容器已经成功运行，此时，你就可以通过你的域名访问到 NextJS 应用了。
...
在 `ci.yml` 文件中，我们使用了 `secrets` 来获取服务器信息。`secrets` 是 Github 仓库的私有变量，用于存储敏感信息。接下来，我们需要在 Github 仓库中配置这些 secrets。
...
> 本文转摘：
>
> 作者：[Jee](https://jeewong.com)
>
> 本文链接：https://jeewong.com/posts/docker-deploying-nextjs
